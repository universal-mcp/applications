...
from universal_mcp.applications.fpl.utils.fixtures import analyze_player_fixtures, get_blank_gameweeks, get_double_gameweeks, get_fixtures_resource, get_player_fixtures, get_player_gameweek_history
from universal_mcp.applications.fpl.utils.helper import find_players_by_name, get_player_info, get_players_resource, get_team_by_name, search_players
from universal_mcp.applications.fpl.utils.league_utils import _get_league_historical_performance, _get_league_standings, _get_league_team_composition, parse_league_standings
...
class FplApp(APIApplication):
    ...
    async def analyze_league(self, league_id: int, analysis_type: str='overview', start_gw: int | None=None, end_gw: int | None=None) -> dict[str, Any]:
        """
        ...
        try:
            ...
            try:
                ...
                return {'error': f'Analysis failed: {str(e)}', 'league_info': league_data['league_info'], 'standings': league_data['standings'], 'status': 'error'}
    ...
    async def get_player_information(self, player_id: int | None=None, player_name: str | None=None, start_gameweek: int | None=None, end_gameweek: int | None=None, include_history: bool=True, include_fixtures: bool=True) -> dict[str, Any]:
        """
    ...
    async def get_gameweek_snapshot(self) -> dict[str, Any]:
        ...
        return {'current_gameweek': current_gw and current_gw['id'], 'current_status': current_status, 'previous_gameweek': previous_gw and previous_gw['id'], 'next_gameweek': next_gw and next_gw['id'], 'season_progress': f"GW {current_gw['id']}/38" if current_gw else 'Unknown', 'exact_timing': {'current_deadline': current_gw and current_gw.get('deadline_time'), 'next_deadline': next_gw and next_gw.get('deadline_time')}}

    async def screen_players(self, position: str | None=None, team: str | None=None, min_price: float | None=None, max_price: float | None=None, min_points: int | None=None, min_ownership: float | None=None, max_ownership: float | None=None, form_threshold: float | None=None, include_gameweeks: bool=False, num_gameweeks: int=5, sort_by: str='total_points', sort_order: str='desc', limit: int=20) -> dict[str, Any]:
        """
            ...
            try:
                ...
                if 'players' in gameweek_data:
                    for player_id, history in gameweek_data['players'].items():
                        ...
                        if not player_info:
                            continue
                        recent_stats = {'player_name': player_info.get('name', 'Unknown'), 'matches': len(history), 'minutes': 0, 'points': 0, 'goals': 0, 'assists': 0, 'clean_sheets': 0, 'bonus': 0, 'expected_goals': 0, 'expected_assists': 0, 'expected_goal_involvements': 0, 'points_per_game': 0, 'gameweeks_analyzed': gameweek_data.get('gameweeks', [])}
                        for gw in history:
                ...
                result['recent_form'] = {'description': f'Stats for the last {num_gameweeks} gameweeks only', 'player_stats': recent_form_stats}
    ...
    async def compare_players(self, player_names: list[str], metrics: list[str]=['total_points', 'form', 'goals_scored', 'assists', 'bonus'], include_gameweeks: bool=False, num_gameweeks: int=5, include_fixture_analysis: bool=True) -> dict[str, Any]:
        """
        ...
        comparison = {'players': {name: {'id': player['id'], 'name': player['name'], 'team': player['team'], 'position': player['position'], 'price': player['price'], 'status': 'available' if player['status'] == 'a' else 'unavailable', 'news': player.get('news', '')} for name, player in players_data.items()}, 'metrics_comparison': {}}
            ...
            try:
                ...
                for name, player in players_data.items():
                    ...
                    if 'players' in player_history and player['id'] in player_history['players']:
                        ...
                        recent_stats = {'matches': len(history), 'minutes': 0, 'points': 0, 'goals': 0, 'assists': 0, 'clean_sheets': 0, 'bonus': 0, 'expected_goals': 0, 'expected_assists': 0, 'expected_goal_involvements': 0, 'points_per_game': 0}
                        for gw in history:
                            recent_stats['minutes'] += gw.get('minutes', 0)
                            recent_stats['points'] += gw.get('points', 0)
                            recent_stats['goals'] += gw.get('goals', 0)
                ...
                if gameweek_comparison:
                    ...
                    comparison['recent_form_comparison'] = {'description': f'Aggregated stats for the last {num_gameweeks} gameweeks only', 'gameweeks_analyzed': gameweek_range, 'player_stats': recent_form_comparison}
                ...
                try:
                    ...
                    for double_gw in double_gws:
                        for team_info in double_gw.get('teams_with_doubles', []):
                            if team_info.get('name') == team_name:
                                double_impact.append({'gameweek': double_gw['gameweek'], 'fixture_count': team_info.get('fixture_count', 2)})
                    double_gameweek_impacts[name] = double_impact
                except Exception:
                    pass
            if fixture_comparison:
                comparison['fixture_comparison'] = {'upcoming_fixtures': fixture_comparison, 'fixture_scores': fixture_scores, 'blank_gameweeks': blank_gameweek_impacts, 'double_gameweeks': double_gameweek_impacts}
                if len(fixture_scores) >= 2:
                    ...
                    comparison['fixture_comparison']['fixture_advantage'] = {'best_fixtures': best_fixtures_player, 'worst_fixtures': worst_fixtures_player, 'advantage': f'{best_fixtures_player} has easier upcoming fixtures than {worst_fixtures_player}'}
        ...
        comparison['summary'] = {'metrics_won': player_wins, 'overall_best': max(player_wins.items(), key=lambda x: x[1])[0] if player_wins else None}
        return comparison
    ...
    async def analyze_entity_fixtures(self, entity_type: str='player', entity_name: str | None=None, num_gameweeks: int=5, include_blanks: bool=True, include_doubles: bool=True) -> dict[str, Any]:
        """
        ...
        result = {'entity_type': entity_type, 'entity_name': entity_name, 'current_gameweek': current_gameweek, 'analysis_range': list(range(current_gameweek + 1, current_gameweek + num_gameweeks + 1))}
        if entity_type == 'player':
            ...
            result['player'] = {'id': player['id'], 'name': player['name'], 'team': player['team'], 'position': player['position'], 'status': 'available' if player['status'] == 'a' else 'unavailable'}
            ...
            result['fixture_analysis'] = {'difficulty_score': round(fixture_score, 1), 'fixtures_analyzed': len(player_fixtures), 'home_matches': sum((1 for f in player_fixtures if f['location'] == 'home')), 'away_matches': sum((1 for f in player_fixtures if f['location'] == 'away'))}
            if fixture_score >= 8:
            ...
            for fixture in upcoming_fixtures:
                ...
                formatted_fixtures.append({'gameweek': fixture['gameweek'], 'opponent': opponent['name'], 'location': 'home' if is_home else 'away', 'difficulty': difficulty})
            ...
            if formatted_fixtures:
                ...
                result['fixture_analysis'] = {'difficulty_score': round(fixture_score, 1), 'fixtures_analyzed': len(formatted_fixtures), 'home_matches': sum((1 for f in formatted_fixtures if f['location'] == 'home')), 'away_matches': sum((1 for f in formatted_fixtures if f['location'] == 'away'))}
                if fixture_score >= 8:
                    result['fixture_analysis']['assessment'] = 'Excellent fixtures'
                elif fixture_score >= 6:
                    result['fixture_analysis']['assessment'] = 'Good fixtures'
            ...
            for team in teams_with_position:
                ...
                for fixture in upcoming_fixtures:
                    ...
                    if is_home or is_away:
                        difficulty = fixture['difficulty']['home' if is_home else 'away']
                        team_fixtures.append({'gameweek': fixture['gameweek'], 'opponent': fixture['away_team']['name'] if is_home else fixture['home_team']['name'], 'location': 'home' if is_home else 'away', 'difficulty': difficulty})
                if team_fixtures:
                    total_diff = sum((f['difficulty'] for f in team_fixtures))
                    avg_diff = total_diff / len(team_fixtures)
                    fixture_score = (6 - avg_diff) * 2
                    team_difficulties[team] = {'fixtures': team_fixtures, 'difficulty_score': round(fixture_score, 1), 'fixtures_analyzed': len(team_fixtures)}
            ...
            if sorted_teams:
                ...
                result['recommendations'] = {'teams_with_best_fixtures': best_teams, 'analysis': f"Teams with players in position {normalized_position} with the best upcoming fixtures: {', '.join(best_teams)}"}
        if include_blanks:
    ...
    def list_tools(self):
        ...
        """
        return [self.get_player_information, self.find_players, self.get_gameweek_snapshot, self.screen_players, self.compare_players, self.analyze_player_fixtures, self.analyze_entity_fixtures, self.get_blank_gameweeks, self.get_double_gameweeks, self.get_league_standings, self.analyze_league, self.get_manager_team_info]