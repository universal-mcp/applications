...
class FirecrawlApp(APIApplication):
    ...
    def _handle_firecrawl_exception(self, e: Exception, operation_desc: str) -> str:
        ...
        is_auth_error = 'unauthorized' in error_str or 'api key' in error_str or 'authentication' in error_str or (hasattr(e, 'response') and hasattr(e.response, 'status_code') and (e.response.status_code == 401)) or (hasattr(e, 'status_code') and e.status_code == 401)
        if is_auth_error:
    ...
    async def quick_web_extract(self, urls: list[str], prompt: str | None=None, schema: Any | None=None, system_prompt: str | None=None, allow_external_links: bool | None=False) -> dict[str, Any]:
        """
        ...
        try:
            client = self._get_client()
            response = client.extract(urls=urls, prompt=prompt, schema=schema, system_prompt=system_prompt, allow_external_links=allow_external_links)
    ...
    def list_tools(self):
        return [self.scrape_url, self.search, self.start_crawl, self.check_crawl_status, self.cancel_crawl, self.start_batch_scrape, self.check_batch_scrape_status, self.quick_web_extract, self.check_extract_status]