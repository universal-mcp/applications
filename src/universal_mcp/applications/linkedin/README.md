...
| `list_profile_comments` | Retrieves a list of comments made by a specific user using their provider ID. |
...
| `list_post_comments` | Fetches comments for a specific post. Providing an optional `comment_id` retrieves threaded replies instead of top-level comments. `retrieve_post` or `list_profile_posts` can be used to obtain the `post_id` which is the social_id in their response. |
...
| `send_invitation` | Sends a connection invitation to a LinkedIn user specified by their provider ID. An optional message and the user's email can be included. |
...
| `cancel_sent_invitation` | Cancels a sent LinkedIn connection invitation that is currently pending. This function performs a DELETE request to remove the invitation, withdrawing the connection request. |
...
| `retrieve_job_posting` | Retrieve a job offer. |
| `publish_job_posting` | Publish the job posting draft you have been working on. |
| `solve_job_publishing_checkpoint` | Solve a checkpoint to verify your member privileges. |
| `list_job_applicants` | Retrieve all the users that have applied to a given offer. |
| `retrieve_job_applicant` | Retrieve the details of a user that has applied to a given offer. Applies to Classic job posting only. |
| `download_job_applicant_resume` | Download the resume of a job applicant. |
